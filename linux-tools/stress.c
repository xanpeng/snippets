// modified by xanpeng, based on 1.0.4
...
#define dbg(OUT, STR, ARGS...) if (global_debug >= 3) \
  fprintf(stdout, "%s: dbug: [%lli] ", global_progname, (long long)getpid()), \
  fprintf(OUT, STR, ##ARGS), \
  fflush(OUT)
...
#define wrn(OUT, STR, ARGS...) if (global_debug >= 1) \
  fprintf(stderr, "%s: WARN: [%lli] (%d) ", global_progname, (long long)getpid(), __LINE__), \
  fprintf(OUT, STR, ##ARGS), \
  fflush(OUT)

#define err(OUT, STR, ARGS...) if (global_debug >= 0) \
  fprintf(stderr, "%s: FAIL: [%lli] (%d) ", global_progname, (long long)getpid(), __LINE__), \
  fprintf(OUT, STR, ##ARGS), \
  fflush(OUT)

/* Implementation of check for option argument correctness.  */
#define assert_arg(A) \
  if (++i == argc || ((arg = argv[i])[0] == '-' && !isdigit ((int)arg[1]) )) { \
...
int version(int status)
{
  ...
  fprintf(stdout, mesg, global_progname, "1.0.4");
  if (status <= 0)
    exit(-1 * status);
