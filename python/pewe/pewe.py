...
# All pewe-generated logs are logged to this namespace.
logger = logging.getLogger('pewe')
...
# By default, pewe supports Sqlite, MySQL and Postgresql.
try:
    import sqlite3
except ImportError:
    try:
...
# --------------------------------------
# fields, Model子类中的字段，分别对应数据库中的字段。
# --------------------------------------
class Field(Node):
    """A column on a table."""
    _field_counter = 0
    ...
    def __init__(self, null=False, index=False, unique=False,
        ...
        self.primary_key = primary_key  # 该field是否是primary key field
    ...
    def add_to_class(self, model_class, name):
        ...
        `Model`.
        钩子函数，将类的`Field`属性替换成命名的`FieldDescriptor`，在Model子类的构建中这个钩子被调用
        name: model定义时的成员变量名
        """
        self.name = name
        ...
        setattr(model_class, name, FieldDescriptor(self))  # model_class的成员变量名关联到FieldDescriptor
        self._is_bound = True
...
class QueryCompiler(object):
    ...
    def _parse_clause(self, node, alias_map, conv):
        sql, params = self.parse_node_list(  # 构建出sql语句（含占位符）和参数列表
    ...
    def generate_insert(self, query):
        # query是InsertQuery实例等
        model = query.model_class  # model子类.__class__
        meta = model._meta
        alias_map = self.alias_map_class()
        alias_map.add(model, model._meta.db_table)  # 如{Person.class: 'person'}
        statement = query._upsert and 'INSERT OR REPLACE INTO' or 'INSERT INTO'
        clauses = [SQL(statement), model.as_entity()]  # 从句都是高层对象
        ...
        if query._query is not None:
            ...
            for row_dict in query._iter_rows():
                ...
                for field in fields:
                    ...
                    if not isinstance(value, (Node, Model)):
                        value = Param(value, conv=field.db_value)  # 将field值构建成Param节点
                    values.append(value)

                value_clauses.append(EnclosedClause(*values))

            if fields:  # 根据fields，补充sql语句的从句(clauses)，clause是sql语句组件的高层语义
                clauses.extend([
    ...
    def return_parsed_node(function_name):
        # TODO: treat all `generate_` functions as returning clauses, instead of SQL/params.
        def inner(self, *args, **kwargs):
...
class Query(Node):
    ...
    def _execute(self):
        sql, params = self.sql()  # 获得含占位符sql语句及参数列表
...
class InsertQuery(Query):
    # model_class: model子类.__class__
    # field_dict: model实例成员及其值
    def __init__(self, model_class, field_dict=None, rows=None,
        ...
        if rows is not None:
            ...
            self._rows = [field_dict or {}]  # 待插入的数据行列表
    ...
    def sql(self):
        """生成sql语句"""
        # self.compiler()获取database（指peewee里的类实例）实例的compiler，都指向QueryCompiler
...
class Database(object):
    ...
    def exception_wrapper(self):
        # 简化try..except成with exception_wrapper?
    ...
    def compiler(self):
        # 初始化时compiler_class = QueryCompiler
    ...
    def execute_sql(self, sql, params=None, require_commit=True):
        # sql: 生成的sql语句，如：
        # 'CREATE TABLE `person` (`id` INTEGER AUTO_INCREMENT NOT NULL PRIMARY KEY,
        #   `name` VARCHAR(255) NOT NULL, `birthday` DATE NOT NULL, `is_relative` BOOL NOT NULL)'
...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        meta_options = {}
        meta = attrs.pop('Meta', None)  # model子类提供Meta子类
        if meta:
            for k, v in meta.__dict__.items():
                if not k.startswith('_'):
                    meta_options[k] = v   # 记录SubModel.Meta的设置

        model_pk = getattr(meta, 'primary_key', None)  # 尝试从Meta中读取primary_key设置
        parent_pk = None  # 父类的primary key

        # inherit any field descriptors by deep copying the underlying field
        # into the attrs of the new model, additionally see if the bases define
        # inheritable model options and swipe them
        # 处理所有父类的_meta和field descriptor
        for b in bases:
            ...
            all_inheritable = cls.inheritable | base_meta._additional_keys  # 取set的并集
            for (k, v) in base_meta.__dict__.items():  # 吸收父类的_meta
            ...
            for (k, v) in b.__dict__.items():
                if k in attrs:
                    continue
                if isinstance(v, FieldDescriptor):  # 继承父类的fields
        ...
        if not cls._meta.db_table:  # 如果没有设置table name，默认用类名.lower()
            cls._meta.db_table = re.sub('[^\w]+', '_', cls.__name__.lower())

        # replace fields with field descriptors, calling the add_to_class hook
        # 将fields替换成field descriptors，并调用钩子add_to_class。
        fields = []
...
class Model(with_metaclass(BaseModel)):
    ...
    @classmethod
    def alias(cls):
        """实现SQL Aliases: http://www.w3schools.com/sql/sql_alias.asp"""
    ...
    def create_table(cls, fail_silently=False):
        ...
        db = cls._meta.database  # 指向model的database实例
        pk = cls._meta.primary_key  # 指向model的主键Field，pk.name
    ...
    def save(self, force_insert=False, only=None):
        # self._data: model子类的“变量名：数据”dict，不包含生成的id（如果有）
        ...
        if self._get_pk_value() is not None and not force_insert:
            ...
            pk = self._get_pk_value()  # 主键的值，新增row的话主键值可能为None（主键是生成的id）
