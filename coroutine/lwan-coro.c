...
void coro_swapcontext(coro_context_t *current, coro_context_t *other) __attribute__((noinline));
    asm(
    ".text\n\t"
    ".p2align 4\n\t"
...
static void coro_entry_point(coro_t *coro, coro_function_t func) {
    int return_value = func(coro);
    coro->ended = true;
    coro_yield(coro, return_value);
}

static void coro_run_deferred(coro_t *coro) {
...
void coro_reset(coro_t *coro, coro_function_t func, void *data) {
    ...
    stack = (unsigned char *)((uintptr_t)(stack + CORO_STACK_MIN - sizeof(uintptr_t) * 2) & 0xfffffff0);
...
ALWAYS_INLINE coro_t* coro_new(coro_switcher_t *switcher, coro_function_t function, void *data) {
...
ALWAYS_INLINE void * coro_get_data(coro_t *coro) {
    return LIKELY(coro) ? coro->data : NULL;
}

ALWAYS_INLINE int coro_resume(coro_t *coro) {
    assert(coro);
    ...
    if (!coro->ended) {
        memcpy(&coro->context, &coro->switcher->callee, sizeof(coro->context));
        memcpy(&coro->switcher->caller, &prev_caller, sizeof(coro->switcher->caller));
    }
#endif
...
ALWAYS_INLINE int coro_resume_value(coro_t *coro, int value) {
    assert(coro);
    assert(coro->ended == false);
...
ALWAYS_INLINE int coro_yield(coro_t *coro, int value) {
    assert(coro);
    coro->yield_value = value;
...
static void coro_defer_any(coro_t *coro, void (*func)(), void *data1, void *data2) {
...
ALWAYS_INLINE void coro_defer(coro_t *coro, void (*func)(void *data), void *data) {
    coro_defer_any(coro, func, data, NULL);
}

ALWAYS_INLINE void coro_defer2(coro_t *coro, void (*func)(void *data1, void *data2), void *data1, void *data2) {
...
inline void * coro_malloc(coro_t *coro, size_t size) {
