...
static int mmap_fault(struct vm_area_struct *area, struct vm_fault *vmf)
{
	struct mmap_info *info;
	pgoff_t offset = vmf->pgoff;
	if (offset > area->vm_end) {
		printk("invalid address\n");
		return VM_FAULT_SIGBUS;
	}
	info = (struct mmap_info *)area->vm_private_data;
	if (!info->data) {
		printk("no data\n");
		return VM_FAULT_SIGBUS;
	}
...
struct vm_operations_struct mmap_vm_ops = {
	.open = mmap_open,
	.close = mmap_close,
	// remove nopage: http://lwn.net/Articles/258113/
	// .nopage = mmap_nopage,
	.fault = mmap_fault,
};
