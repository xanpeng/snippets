...
coro_t* coro_new(coro_switcher_t *switcher, coro_function_t function, void *data); // 仅仅创建coro，并未立即执行
void coro_free(coro_t *coro);  // 执行deferred，然后free

void coro_reset(coro_t *coro, coro_function_t func, void *data);

int coro_resume(coro_t *coro);  // 恢复coro的执行
int coro_resume_value(coro_t *coro, int value);
int coro_yield(coro_t *coro, int value);  // 切换到调用当前coro的context去执行

void *coro_get_data(coro_t *coro);

void coro_defer(coro_t *coro, void (*func)(void *data), void *data); // 设置coro->defer
void coro_defer2(coro_t *coro, void (*func)(void *data1, void *data2), void *data1, void *data2);
void *coro_malloc(coro_t *coro, size_t sz);  // malloc defer_t，设置coro->defer
